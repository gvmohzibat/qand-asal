...
const RGB DGRAY(30,30,30);

class ImageLoad{
public:
	ImageLoad(string add, SDL_Surface* surface) : address(add), sdl_surface(surface) {};
	string address;
...
public:
	...
	void draw_bmp(string filename, int x, int y, int, int);
	...
	SDL_Surface* find_in_cache(string add);
	void update_screen();
private:
	SDL_Window* win;	
	SDL_Renderer* renderer;
	SDL_Surface* surface;
	vector<ImageLoad*> bmp_cache;
